<script lang="ts">
    ...
    let { content, image, timeSince, isResponse = false }: IResponseItem = $props();
</script>
...
{#if isResponse}
    <div class="chat chat-start group">
        ...
        <div class="chat-bubble bg-base-200 relative">
            {content}
            {#if image}
                <img src={image} class="rounded-box mt-1" alt="Gen Image" />
            {/if}
            <div
                ...
                <button class="btn btn-xs">Regenerate</button>
        ...
        <div class="chat-footer opacity-50">{timeSince}</div>
    </div>
{:else}
